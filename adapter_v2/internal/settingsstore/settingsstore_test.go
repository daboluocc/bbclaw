...
func TestExportEnv(t *testing.T) {
	...
	s.CLI.ConfirmOnDevice = true  // page-set true must be exported as "1"
	...
	if got := os.Getenv("ADAPTER_V2_CONFIRM_ON_DEVICE"); got != "1" {
		t.Errorf("ADAPTER_V2_CONFIRM_ON_DEVICE = %q, want 1", got)
	}
...
func TestFromEnvDefaults(t *testing.T) {
	for _, k := range []string{
		...
		"ADAPTER_V2_CLAUDE_AUTO_ENTER", "ADAPTER_V2_CONFIRM_ON_DEVICE",
	} {
	...
	if s.CLI.ConfirmOnDevice {
		t.Errorf("ConfirmOnDevice default = true, want false")
	}

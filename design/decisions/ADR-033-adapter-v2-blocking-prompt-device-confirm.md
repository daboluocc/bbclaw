...
- **日期**: 2026-06-24
- **状态**: 进行中（设计定稿；两个 P0 探针**已真机验证 2026-06-24**：数字直提成立、需显式 `--permission-mode default` 才弹）。**P0（adapter 核心）+ P1（devicews LAN）+ P2-a（cloudrelay parked-turn）已实现并经多智能体对抗审查 + 修复**：识别器 `extract/prompt.go` + boundary §0 勘误 + deviceapi 转发/注入/超时 auto-DENY + settingsstore/butler 接线（opt-in `ConfirmOnDevice` 默认 off）+ devicews `prompt.open`/`prompt.select`/`prompt.close` 端到端回路 + cloudrelay `voice.prompt.*` 转发 + parked-turn（弹窗 pending 不超时不 supersede）+ `prompt.select` 独立 request kind + `toolApproval` 能力门控；`go test -race ./...` 全绿。**P2-c firmware（LAN bbwire/2）已实现**：`bb_adapter_client.h` 事件 + `bb_prompt_t`（#6 定结构化字段）、`bb_bbwire2.c` cJSON 解析 `prompt.open`/`close` + 发 `prompt.select`、新 `bb_page_prompt_select` LVGL 确认页（仿 ota_confirm，选中默认落拒绝项 §11、倒计时 auto-deny）、`bb_radio_app.c` 路由；`make build`(idf.py) 通过，sim `--mode prompt_select` 目视确认布局 + 安全默认。**待做**：P2-b cloud hub 转发（reference 私有仓，附加 Go）+ 固件 cloud-client（voice.prompt.*）路径；真机 flash + LAN 端到端验证（设备未连，待接）；P0-5 upsell/trust 配置压制（暂由 claudeStartupKeys/warmup 兜底）。
- **关联**:

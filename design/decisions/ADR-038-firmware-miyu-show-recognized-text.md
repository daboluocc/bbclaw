...
## 相关：数字口令歧义（云端归一化，2026-06-25 已修）

显示识别文本暴露了一个独立的**匹配** bug：口令设为数字 `123456`，但 ASR 把口语念的数字识别成中文/大写（`一二三四五六` / `壹贰叁肆伍陆`），字面 ≠ `123456`，永远验不过。修在**云端**（passphrase 比对处，非固件）：`cloud/internal/voiceprint/stub.go` 的 `normalizeTranscript` 增加单字数字归一——中文(简/繁/大写)、口语别名(两/幺/洞)、全角数字 → ASCII 数字，对 transcript 与 expectedPhrase **两边同样处理**（故不引入假阴性）；只映单字，不解析复合数（口令逐位念）。仅云端改动，`make deploy-cloud` 部署，不需固件 tag。固件侧（本 ADR 的识别文本展示）配合它，用户能看到「听到『一二三四五六』」并理解为何匹配/不匹配。
...
- 配套云端数字归一化（见上），数字口令现在能匹配中文/大写识别结果。

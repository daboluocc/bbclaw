 ...
 * no theme rendering, just the three configuration rows:
 *
 *     Agent:      <driver name>
 *     TTS reply:  On / Off
 *     Back
 ...
 *   LEFT/RIGHT : on rows 0..1, preview the row's value (no NVS write).
 ...
 *                then advance cursor. On Back row, OK exits the overlay.
 ...
 *                                   list fetch. Caller must hold the LVGL lock.

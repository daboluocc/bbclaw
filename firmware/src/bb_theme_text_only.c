...
static void refresh_topbar(void) {
  ...
  char buf[128];
  /* ASCII-only flag suffix for WiFi connectivity. Long-mode is DOTS on the
   * topbar label, so overflow truncates with "..." rather than wrapping. */
...
static void theme_on_enter(lv_obj_t* parent) {
  ...
  lv_obj_set_style_pad_right(s_st.topbar_lbl, SCREEN_CORNER_INSET_X, 0);

...
/* ADR-040 — show「未发送」in the topbar for a turn the user cancelled/interrupted
 * before its reply landed. Call AFTER agent_chat_voice_post_error (which restores
 * the session id / clears the listening hint) so this hint wins. */
static void agent_chat_voice_unsent_locked(void) {
  if (lvgl_port_lock(200)) {
    bb_ui_agent_chat_voice_unsent();
    lvgl_port_unlock();
  } else {
    ESP_LOGW(TAG, "agent_chat_voice_unsent_locked: lvgl_port_lock timeout, topbar may lag");
  }
}
...
static void stream_task(void* arg) {
  ...
  /* ADR-012: page state machine drives all nav/PTT dispatch. No more
   * auto-summon (PTT/nav opening chat from standby) and no more 90 s
   * idle-exit. Page transitions are user-explicit via OK/BACK. */
      ...
      if (!skip_finish) {
        ...
        if (finish_err == ESP_OK) {
          ...
          if (ui_stream != NULL && ui_stream->transcript[0] != '\0') {
            ...
            /* 这一句留在 transcript 里(与前后句拼在一起),但它的发送→回复闭环被
             * 撤销/打断了。顶栏说话状态显示「未发送」,让该状态可见(ADR-040)。 */
            agent_chat_voice_unsent_locked();
          } else {
